...
$query = "SELECT `expediente`.*,`datosPer`.`nombre`,`especialidad`.`descripEsp` FROM (`expediente` 
INNER JOIN `datosPer` ON `expediente`.`cedulaExp`=`datosPer`.`cedulaPer`) INNER JOIN `especialidad` ON `expediente`.`codEsp`=`especialidad`.`codEsp` ORDER BY fechaAbre ASC";
...
$titles = array(
				'numExp'=>'<b>Num.</b>',
				'cedulaExp'=>'<b>Cedula</b>',
				'nombre'=>'<b>Nombre y Apellido</b>',
				'descripEsp'=>'<b>Especialidad</b>'
			);
$options = array(
  ...
  $h=100;
  $factor = $height/$h;  
  $pdf->ezImage("../Imagenes/banner1.jpg", 0, 910, 'none', 'center');
  $text = 'SISGEAC';
  $ts=100*$factor;
...
$txttit.= "Reporte de Expedientes archivados a la fecha. \n";
